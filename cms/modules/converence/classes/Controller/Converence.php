...
class Controller_Converence extends Controller_Backend {
	...
	public function action_index() {
		// Redirect
		$this->redirect('/converence/search');
	}
	...
	public function action_search() {
		...
		$data['publish'] = $publish = $session->get("publish_news", "convPublishTime");
		...
		$data['main_title'] = __('Converence List');
		$data['menu_active'] = 'events';
		$data['menu_active_child'] = 'converence';
					...
					$(\'#reservation\').daterangepicker(
						{
							...
							window.location = "' . URL::Base() . 'converence/search/?date_range=" + $(\'#reservation\').val();
						}
					);
				});
			</script>
		...
		$data['custom_footer'] .= '
			<script type="text/javascript">
				function del_confirm(id) {
					...
					if (dialog == true) {
						window.location.href="' . URL::Base() . 'converence/delete/" + id;
					}
				}
			</script>
		';
		...
		// Count all data
		$count_all = $converence_model->count_search($date1, $date2, $search);
		...
		$pagination = 	Pagination::factory(array(
							...
							'items_per_page'		=> 20,
							'current_page'			=> $page,
							'base_url'				=> '/converence/search/',
							'view'					=> 'pagination/admin'
							// 'suffix'				=> '?date_range=' . $date_range,
						));

		$data['list'] = $converence_model->list_search($date1, $date2, $search, $pagination->items_per_page, $pagination->offset);
		...
		$view = Briliant::admin_template('converence/' . Kohana::$config->load('path.main_template') . '/list', $data);
	...
	public function action_new() {
		$data['main_title'] = __('Converence | Add New Data');
		$data['menu_active'] = 'events';
		$data['menu_active_child'] = 'converence';
		...
		$view = Briliant::admin_template('converence/' . Kohana::$config->load('path.main_template') . '/new', $data);
		$this->response->body($view);
	}
	...
	public function action_save() {
		...
		// Validation
		$validation = Converence::validation($this->request->post());
		...
		if($validation !== TRUE) { // Error Validation
			...
			// Save data
			$save_data = $converence_model->save_data($this->request->post(), $user_id);
	...
	public function action_edit() {
		...
		$data['main_title'] = __('Article | Edit Data');
		$data['menu_active'] = 'events';
		$data['menu_active_child'] = 'converence';
		...
		// Detail Data
		$converence_detail = $converence_model->detail_data($id);

		$data['detail'] = $converence_detail;
		$data['detail']['publishTime'] = date("d/m/Y H:i:s", strtotime($data['detail']['publishTime']));
		$view = Briliant::admin_template('converence/' . Kohana::$config->load('path.main_template') . '/edit', $data);
		$this->response->body($view);
	...
	public function action_update() {
		...
		if($validation !== TRUE) { // Error Validation
			...
			$data['main_title'] = __('Converence | Edit Data');
			$data['menu_active'] = 'events';
			$data['menu_active_child'] = 'converence';
			...
			// Update Data
			$update_data = $converence_model->update_data($this->request->post(), $user_id);
	...
	public function action_delete() {
		...
		// Change status to 0
		$converence_model->delete_data($id);
	...
	public function action_detail() {
		...
		$data['main_title'] = __('Converence | Detail');
		$data['menu_active'] = 'events';
		$data['menu_active_child'] = 'converence';
		...
		// Detail data
		$data['detail'] = $converence_model->detail_data($id);

		$member = $session->get('member');
		$view = Briliant::admin_template('converence/' . Kohana::$config->load('path.main_template') . '/detail', $data);
		$this->response->body($view);
